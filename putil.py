...
# uses Axesgrid but nt os much use
class Plm3(object):
		
	def __init__(self,rows=1,cols=1,xsize=7.0,ysize=7.0,xmulti=False,ymulti=False,full=False,slabelx=0.89,slabely=0.1,slabel=True,order='left-right'):
		self.slabels=['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z']
		params = {'font.size':12,
				  'text.usetex':False,
				  'ytick.labelsize': 'medium',
		...
		self.axes = AxesGrid(self.fig, 111,
	...
	def next(self):        
		...
		if type(self.order) == list:
			...
			if self.order == 'top-down':
				...
				self.panelno=self.figno
		self.ax=self.axes[self.panelno-1]
	...
	def figcolorbar(self,*args,**kwargs):
		plt.colorbar(*args,cax=self.ax.cax,**kwargs)
...
class Plm(object):
	...
	def __init__(self,rows=1,cols=1,xsize=7.0,ysize=7.0,xmulti=False,ymulti=False,full=False):
		self.flabels=['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p']
	...
	def next(self):        
		ax=plt.subplot(self.rows,self.cols,self.figno); self.figno+=1
		if (self.slabel) and (self.rows*self.cols > 1):
			plt.text(0.9,0.1,'('+self.flabels[self.figno-2]+')',transform=ax.transAxes)
		if self.xmulti:
			if (self.figno-2)/self.cols != (self.rows-1):
				ax.set_xticks([])
		if self.ymulti:
			if (self.figno-2) % self.cols != 0:
				ax.set_yticks([])
